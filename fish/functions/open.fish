...
function open -d "ファイル・ディレクトリ・URLをWindows側で開く"
    ...
    for target in $argv
        ...
        # - ディレクトリ → explorer.exe（高速）
        # - URL → wslview（安定）
        # - ファイル → wslview（関連付け処理が優秀）
        ...
        if test "$use_wslview" = true
            ...
            # explorer.exeはURLを直接処理できないため、cmd.exe経由で開く
            if test "$is_url" = true
                ...
                # ファイルもしくはディレクトリの場合はWindowsパスに変換して開く
