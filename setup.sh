...
configure_dotfiles() {
  ...
  if [ -f "fish/config.fish" ]; then
    if prompt_overwrite ~/.config/fish/config.fish; then
  ...
  if [ -f "ssh/config" ]; then
    if prompt_overwrite ~/.ssh/config; then
  ...
  if [ -f "ssh/github/config" ]; then
    if prompt_overwrite ~/.ssh/github/config; then
  ...
  if [ -f "mise/config.toml" ]; then
    ...
    log "更新しました：~/.config/mise/config.toml"
  else
